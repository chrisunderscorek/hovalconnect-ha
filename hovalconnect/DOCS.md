...
Energy counters for heat output and inverter energy use are exposed as Home Assistant `kWh` energy sensors. Hoval's cloud live values are converted from values that behave like `MWh` to `kWh`; the original raw value and inferred raw unit remain visible as sensor attributes. Current heat output is exposed separately as a `kW` power sensor. The integration also derives `Arbeitszahl gesamt` / `Total performance factor` from `heatAmount / totalEnergy`; this is a cumulative working factor, not an instantaneous COP.
